...
public class StatisticUtils {
    ...
    public List<Map<String, Object>> getCourseStatisticalByCourseName(String courseName) {
        ...
        String sql = "SELECT " +
                "  sum(CASE when score<60 then 1 else 0 end)   AS 'Dưới trung bình', " +
                "  sum(CASE when score>=60 and score<70 then 1 else 0 end)   AS 'Trung bình', " +
                "  sum(CASE when score>=70 and score<80 then 1 else 0 end)   AS 'Khá', " +
                "  sum(CASE when score>=80 and score<90 then 1 else 0 end)   AS 'Giỏi', " +
                "  sum(CASE when score>=90 and score<=100 then 1 else 0 end)   AS 'Xuất sắc' " +
